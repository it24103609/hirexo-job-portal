                        ...
                        <tr key={contact._id} style={{ background: selectedContact?._id === contact._id ? 'rgba(26,138,86,0.07)' : undefined }}>
                          ...
                          <td>
                            <div className="form-links">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => onStatusChange(contact._id, 'read')}><UserCheck size={13} style={{marginRight: 4}} /> Mark read</Button>
                              <Button size="sm" variant="ghost" onClick={() => onStatusChange(contact._id, 'replied')}><Reply size={13} style={{marginRight: 4}} /> Mark replied</Button>
                              <Button size="sm" variant="danger" disabled={deletingId === contact._id} onClick={() => onDelete(contact._id)}>
                                {deletingId === contact._id ? 'Deleting...' : <><UserX size={13} style={{marginRight: 4}} /> Delete</>}
                              </Button>
                            </div>
                          </td>
                        </tr>
              ...
              <Card style={{ padding: '1.5rem 1.2rem', minWidth: 0 }}>
                ...
                <div className="form-links mt-1" style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <Textarea
                    ...
                    rows={3}
                  />
                  <Button onClick={onReply} disabled={replying || !replyMessage.trim()} variant="primary">{replying ? 'Sending...' : <><Reply size={15} style={{marginRight: 5}} /> Send Reply</>}</Button>
                </div>
              </Card>
            ) : null}
          </div>
